...
Mode Mode::vanityFile(const std::string filename, int t, int w) {
	Mode r;
	r.name = "vanity_file";
	r.kernel = "profanity_score_vanity";
	r.vanity_filename = filename;
	r.prefix_len = t;
	r.suffix_len = w;
	return r;
}
...
std::string Mode::transformKernel() const {
	switch (this->target) {
		...
		default:
			throw std::runtime_error("No kernel for target");
	}
}
...
std::string Mode::transformName() const {
	switch (this->target) {
		...
		case CONTRACT:
			return "Contract";
		default:
			throw std::runtime_error("No name for target");
	}
}
